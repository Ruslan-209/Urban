...
class Bank:
    def __init__(self):
        self.balance = 0 # изначально наш банк пуст
        self.lock = threading.Lock()

    def deposit(self):
        for i in range(1, 101):  # Будет совершать 100 транзакций пополнения средств.
            # print(i)
            replenishment = random.randint(50, 500)  # случайное целое число от 50 до 500.
            with self.lock:
    ...
    def take(self):
        for i in range(1, 101):  # Будет совершать 100 транзакций снятия.
            # print(i)
            removal = random.randint(50, 500)  # случайное целое число от 50 до 500
            print(f"Запрос на {removal}", flush=True)
            with self.lock:
                if removal <= self.balance:  # производится проверка: если случайное число меньше или равно текущему балансу, то произвести снятие
                    self.balance -= removal # снятие с баланса
                    print(f"Снятие: {removal}. Баланс: {self.balance}", flush=True)
                else:
                    print("Запрос отклонён, недостаточно средств", flush=True)
                    # self.lock.acquire()  # Блокировка потока, если средств недостаточно
