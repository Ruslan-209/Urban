...
def all_variants(text):
    for i in range(1, len(text) + 1):  # Перебираем длины подстрок от 1 до длины строки
        for j in range(len(text) - i + 1):  # Начальная позиция подстроки
            yield text[j : j + i]  # Генерируем подстроку заданной длины
